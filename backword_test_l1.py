...
""""Первый слой"""
weight_l1_n1 = torch.tensor([0.6235637, -0.61561829], requires_grad=True)
input_l1_n1 = torch.tensor([2.0037346, 1.])
pr_l1_n1 = il_sigma(torch.sum(weight_l1_n1 * input_l1_n1)).unsqueeze(dim=0)
pr_l1_n1.requires_grad_(True)

weight_l1_n2 = torch.tensor([0.05671298, -0.72734371], requires_grad=True)
input_l1_n2 = torch.tensor([2.0037346, 1.])
pr_l1_n2 = il_sigma(torch.sum(weight_l1_n2 * input_l1_n2)).unsqueeze(dim=0)
...
print(f"градиент от pytorch: {weight_l1_n1.grad}")
print(f"градиент от pytorch: {weight_l1_n2.grad}")
""""  Ошибки нет! проверил два случая!
"""
...
# градиент от pytorch: tensor([0.0029, 0.0014])
# градиент от pytorch: tensor([-0.0077, -0.0039])

# y_true = torch.tensor([0, 1]).unsqueeze(dim=0)
# Log_loss:  1.7161902
# pr_1: 1.5214 sf_1: 0.8203, soft_max 0.8202503323554993
# pr_2: 0.0034 sf_2: 0.1797, soft_max 0.17974966764450073
# градиент от pytorch: tensor([-0.0131, -0.0065])
# градиент от pytorch: tensor([0.0353, 0.0176]) "" тут ошибка!!!!!!!!""